...
class PolarizedTuplesBag:
    def __init__(self, tuples=None):
        if tuples is None:
            self.tuples = tuples
            self.built = False
        elif type(tuples) is dict:
            self.tuples = tuples
            self.built = True
        else:
            raise ValueError
    
    def build(self, tuples, targets):
        assert not self.built
        self.tuples = defaultdict(set)
        for t in tuples:
            self.tuples[tuple(tg(t) for tg in targets)].add(t)
        self.tuples = dict(self.tuples)
